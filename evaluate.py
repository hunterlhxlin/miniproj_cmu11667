...
def predict_sft(model, tokenizer, dataset, batch_size=8):
    ...
    for i in tqdm(range(num_batches)):
        ...
        generated_texts = [tokenizer.decode(generated_ids[0][inputs_len:], skip_special_tokens=True)]
        ...
        for j, text in enumerate(generated_texts):
            ...
            if "tie (bothbad)" in text:
                ...
                predicted_label = "tie"
            elif "model_a" in text or " a" in text:
                predicted_label = "model_a"
            elif "model_b" in text or " b" in text:
...
def predict_icl(model, tokenizer, dataset, few_shot_examples, N=4):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        prompts = generate_few_shot_prompts(few_shot_examples, ex["conversation_a"], ex["conversation_b"])
        inputs = tokenizer(prompts, return_tensors='pt').to(device)
        inputs_token_size = inputs['input_ids'].shape[1]
        ...
        with torch.no_grad():
            ...
            output_text = tokenizer.decode(output_ids[0][inputs_token_size:], skip_special_tokens=True)

        chosen_label = "tie"
        output_text_lower = output_text.split('\n')[0].lower()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default="./data_split", help="Path to the split dataset directory.")
    parser.add_argument("--model_dir", type=str, default="meta-llama/Llama-3.2-1B-Instruct",
